...
export default function N8nLessonLayout({ slug, children }: Props) {
  const index = n8nLessons.findIndex((l) => l.slug === slug);
  const prev = index > 0 ? n8nLessons[index - 1] : null;
  const next = index < n8nLessons.length - 1 ? n8nLessons[index + 1] : null;

  return (
    <div className="max-w-3xl mx-auto flex flex-col gap-10">
      <p className="text-xs text-gray-500 uppercase tracking-widest">
        n8n &rsaquo; {n8nLessons[index].label}
      </p>
