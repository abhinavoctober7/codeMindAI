...
export default function LangChainLessonLayout({ slug, children }: Props) {
  const index = langchainLessons.findIndex((l) => l.slug === slug);
  const prev = index > 0 ? langchainLessons[index - 1] : null;
  const next = index < langchainLessons.length - 1 ? langchainLessons[index + 1] : null;

  return (
    <div className="max-w-3xl mx-auto flex flex-col gap-10">
      <p className="text-xs text-gray-500 uppercase tracking-widest">
        LangChain &rsaquo; {langchainLessons[index].label}
      </p>
