...
export default function JavaScriptLessonLayout({ slug, children }: Props) {
  const index = javascriptLessons.findIndex((l) => l.slug === slug);
  const prev = index > 0 ? javascriptLessons[index - 1] : null;
  const next = index < javascriptLessons.length - 1 ? javascriptLessons[index + 1] : null;

  return (
    <div className="max-w-3xl mx-auto flex flex-col gap-10">
      <p className="text-xs text-gray-500 uppercase tracking-widest">
        JavaScript &rsaquo; {javascriptLessons[index].label}
      </p>
      ...
      <div className="flex items-center justify-between border-t border-white/10 pt-6 mt-2">
        {prev ? (
          <Link
            href={getJsLessonHref(prev.slug)}
        ...
        {next ? (
          <Link
            href={getJsLessonHref(next.slug)}
